...
public class Fall {
	 ...
	 * @param positionY
	 */
	public void updatePositionMap(final int positionX, final int positionY,final Map<String, Integer> fallPositionMap) {
	...
	public Boolean isFall(final Tetorimino tetorimino,final Map<String, Integer> fallPositionMap) {
		...
		for (int i = 0; i < tetorimino.shape().length; i++) {
			for (int j = 0; j < tetorimino.shape()[i].length; j++) {
				...
				if (tetorimino.shape()[i][j]!=0&&(fallPositionMap.get(
						String.valueOf( (j + tetorimino.getPositionX())+"@"+(i
								+ tetorimino.getPositionY()+1))).intValue() != 0||(i+tetorimino.getPositionY())==this.MAX_DEPTH)) {
					check = true;
	 ...
	 * 列削除処理によるポジションマップの更新
	 */
	public void addFallPositionMap(final int deleteBlock, final int miniCount,final Map<String, Integer> fallPositionMap) {
		if (deleteBlock != 0) {
			for (String key : fallPositionMap.keySet()) {

				fallPositionMap.put(key, new Integer(deleteBlock
						+ fallPositionMap.get(key).intValue()));
