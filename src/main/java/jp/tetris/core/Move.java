...
public class Move {
	 ...
	 * テトリミノ右移動
	 */
	public void rightMoveTetoriminoe(final Tetorimino moveTetorimino,final int FIELD_WIDTH,final Map<String, Integer> fallPositionMap) {
		
			Boolean moveOk = true;
			for (int i = 0; i < moveTetorimino.shape().length; i++) {
				for (int j = 0; j < moveTetorimino.shape()[i].length; j++) {
					if (moveTetorimino.shape()[i][j]!=0&&(fallPositionMap.get(
							String.valueOf( (j + moveTetorimino.getPositionX())+1+"@"+(i
									+ moveTetorimino.getPositionY()))).intValue() != 0)) {
						moveOk = false;
					}
				}
			}
